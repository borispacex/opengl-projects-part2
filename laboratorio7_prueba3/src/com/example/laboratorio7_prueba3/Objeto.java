package com.example.laboratorio7_prueba3;
...
public class Objeto {
	...
	public void dibuja(GL10 gl) {
		...
		while (iterador.hasNext()) {
			...
			/* Especifica los datos del arreglo de las normales */
			gl.glNormalPointer(GL10.GL_FLOAT, 0, bufNormales);
			
			/* Activa el arreglo de las normales del lado del cliente */
			gl.glEnableClientState(GL10.GL_NORMAL_ARRAY);
	
			/* Renderiza las primitivas desde los datos de los arreglos (vértices e indices) */
			...
			/* Desactiva el arreglo de las normales del lado del cliente */
			...
			while ((linea = buffer.readLine()) != null){
				...
				if (linea.length() > 0) {
					if (linea.startsWith("mtllib ")) {			/* nombre del arch. de materiales */
						...
						grupos.get(indiceDeGrupo).adiTriangulo(numAristas);
						numAristas++;
