...
public class Elipsoide {
	...
	public Elipsoide(float radio, int segmentosH, int segmentosV) {
		...
		for (float phi = 0; phi < 360; phi += inc_phi) {
			...
			for (float theta = 0; theta < 180; theta += inc_theta) {
				float st = (float) Math.sin(Math.toRadians(theta))*2;
				float ct = (float) Math.cos(Math.toRadians(theta));
				float st1 = (float) Math.sin(Math.toRadians(theta + inc_theta))*2;
